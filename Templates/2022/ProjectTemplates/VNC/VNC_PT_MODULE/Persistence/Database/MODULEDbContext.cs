...
{
    public class $xxxMODULExxx$DbContext : DbContext, I$xxxMODULExxx$DbContext
    {
        // TODO(crhodes)
        ...
        public $xxxMODULExxx$DbContext() : base("$xxxMODULExxx$_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_CATEGORY);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<$xxxMODULExxx$DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<$xxxMODULExxx$DbContext>(new CreateDatabaseIfNotExists<$xxxMODULExxx$DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<$xxxMODULExxx$DbContext>(new DropCreateDatabaseIfModelChanges<$xxxMODULExxx$DbContext>());

            Database.SetInitializer<$xxxMODULExxx$DbContext>(
                new DropCreateDatabaseIfModelChanges<$xxxMODULExxx$DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<$xxxMODULExxx$DbContext>(new DropCreateDatabaseAlways<$xxxMODULExxx$DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<$xxxMODULExxx$DbContext>(new $xxxMODULExxx$DbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"($xxxAPPLICATIONxxx$DbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("($xxxMODULExxx$DbContext) Enter", Common.LOG_CATEGORY);
            ...
            try
            {
                ...
                Log.PERSISTENCE($"($xxxMODULExxx$DbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
